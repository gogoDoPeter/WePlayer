...
#ifndef WEPLAYER_SAFE_QUEUE_H
#define WEPLAYER_SAFE_QUEUE_H
...
private:
    queue<T> queue;
    pthread_mutex_t mutex;// 互斥锁 为了多线程访问安全
    pthread_cond_t cond;//信号量 做等待 和 唤醒
    int work;//标记队列是否工作
...
public:
    ...
    //virtual ~SafeQueue() {} 看Derry一般删除virtual，为什么，有什么差异？
    //父类的析构函数必须带virtual
    ~SafeQueue() {
    ...
    */
    ...
    int empty(){    //如果引用的容器为空，则该方法返回“ true”，否则返回“ false”, 函数语法：  bool empty() const;
        return queue.empty();
    }
